...
function decodeShaderSourceCode(e) {
  ...
  for (let s = 0; s < 256; s++) o[s] = s;

  for (var c = 0, u = 0; u < 256; u++) {
    const l = [o[c = (c + o[u] + t[u % n]) % 256], o[u]];
    o[u] = l[0], o[c] = l[1];
  }
  let d; let
    f = 0;
  c = 0;
  for (let p = 0; p < 0 + i; p++) {
    const m = [o[c = (c + o[f = (f + 1) % 256]) % 256], o[f]];
    o[f] = m[0], o[c] = m[1], d = o[(o[f] + o[c]) % 256], p >= 0 && (a[p - 0] = r[p - 0] ^ d);
  }
  const g = String.fromCharCode.apply(null, a);
...
let BeautyEffectEnabled = false;
let BeautyEffectSingleton = null;
...
BeautyEffect.getInstance = () => new BeautyEffect();

BeautyEffect.getDefaultOpts = () => ({
  denoiseLevel: 5,
  lightLevel: 0.35,
  rednessLevel: 0.5,
  contrastLevel: 1,
...
function Fe(e, t, n) {
  ...
  this.program = createCustomProgram(r, Ve), this.setUniforms = function () {
    ...
    const C = r.getUniformLocation(this.program, 'u_originImage');
    r.activeTexture(r.TEXTURE2);
    r.bindTexture(r.TEXTURE_2D, e.inputTexture);
    r.uniform1i(C, 2);
    for (let w = ['lighten_lut'], R = [r.TEXTURE3], k = 0; k < w.length; k++) {
      const A = r.getUniformLocation(this.program, w[k]);
  ...
  this._updateRedness = function (e) {
    let t; let n; const r = 3.141593;
    let i = e;
    let a = i;
    i > 1 && (i = 1), i < 0 && (i = 0), a > 1 && (a = 1), a < 0 && (a = 0), f = 0.8 * i, a < 0.8 && (a = 0), (d = (117 - 4 * a) / 180 * r) < (l = (116 - 4 * a) / 180 * r) && (d = l), l < r / 2 && (l = r / 2), d < r / 2 && (d = r / 2), (c = (115 - 4 * a) / 180 * r) < r / 2 && (c = r / 2), (u = 173 / 180 * r) > r && (u = r), d > r && (d = r), (s = 175 / 180 * r) > r && (s = r), n = u - d, m = (t = s - u) > 0.01 ? f * n / t : f, n = d - l, p = (t = l - c) > 0.01 ? f * n / t : f;
  };

  this._updateLut = function (e) {
    let t = null;
    e == 0 && (t = h), e == 1 && (t = y), e == 2 && (t = v);
    for (let n = [new Uint8Array(t)], i = [256], a = [1], o = 0; o < n.length; o++) r.bindTexture(r.TEXTURE_2D, g[o]), r.texImage2D(r.TEXTURE_2D, 0, r.LUMINANCE, i[o], a[o], 0, r.LUMINANCE, r.UNSIGNED_BYTE, n[o]);
  };

  this._init = function () {
    for (let e = [new Uint8Array(h)], t = [256], n = [1], i = 0; i < e.length; i++) {
...
Object.assign(BeautyEffect.prototype, {
  init(videoWidth, videoHeight, context) {
    if (typeof WebGLRenderingContext === 'undefined') throw new Error('NOT_SUPPORT_WEBGL');
    this.gl = context;
  ...
  _initGL(videoWidth, videoHeight) {
    const { gl } = this;
...
function genMethod(methodName, paramName, methodCall) {
  BeautyEffect.prototype[methodName] = function (arg1, arg2) {
    this.programs.forEach((customProgram) => {
      if (methodCall) {
...
glUtil.renderWithWebGL = (context, width, height, srcObject, enableBeautyEffect, level) => (
  new Promise(((resolve) => {
    console.log(level);
    ...
    if (glUtil.isInit) {
      ...
      beautyEffect.init(width, height, context);
      if (enableBeautyEffect) {
        beautyEffect.setEnableBeauty(true);
      };
      glUtil.isInit = false;
    }
    const opts = BeautyEffect.getDefaultOpts();
    beautyEffect.setDenoiseLevel(opts.denoiseLevel * level * 10);
    beautyEffect.setLightLevel(opts.lightLevel * level / 5);
    beautyEffect.setRednessLevel(opts.rednessLevel);
    beautyEffect.setContrastLevel(opts.contrastLevel * level);
    beautyEffect.render(srcObject, width, height);
    const pixels = new Uint8ClampedArray(width * height * 4);
    const { gl } = beautyEffect;
